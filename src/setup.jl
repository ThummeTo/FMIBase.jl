...
function setupSolver!(fmu::FMU, tspan, kwargs)
    ...
    if isnothing(t_start)
        ...
        if isnothing(t_start)
            ...
            warn(
                fmu,
                "No `t_start` given, no `t_start` available in the FMU model description, auto-picked `t_start=0.0`.",
            )
        end
    end
    ...
    if isnothing(t_stop)
        ...
        if isnothing(t_stop)
            ...
            warn(
                fmu,
                "No `t_stop` given, no `t_stop` available in the FMU model description, auto-picked `t_stop=1.0`.",
            )
        end
    end
...
function setupCallbacks(
    ...
    if c.fmu.hasTimeEvents && c.fmu.executionConfig.handleTimeEvents
        timeEventCb = IterativeCallback(
            (integrator) -> time_choice(c, integrator, t_start, t_stop),
            (integrator) -> affectFMU!(c, integrator, 0, _inputFunction),
            Float64;
    ...
    if savingValues
        dtypes = collect(
            dataTypeForValueReference(c.fmu.modelDescription, vr) for vr in recordValues
        )
        ...
        if saveat === nothing
            savingCB = SavingCallback(
                (u, t, integrator) ->
                    saveValues(c, recordValues, u, t, integrator, _inputFunction),
                c.solution.values,
            )
        else
            savingCB = SavingCallback(
    ...
    if savingEventIndicators
        ...
        if saveat === nothing
            savingCB = SavingCallback(
                (u, t, integrator) -> saveEventIndicators(
                    c,
                    recordEventIndicators,
                    u,
                    t,
    ...
    if recordEigenvalues
        ...
        if saveat === nothing
            savingCB = SavingCallback(
                (u, t, integrator) ->
                    saveEigenvalues(c, u, t, integrator, _inputFunction),
